...
data-platform-api-invoice-document-items-creates-subfunc-rmq-kube は、データ連携基盤において、入出荷APIサービスのヘッダ登録補助機能を担うマイクロサービスです。
...
* InvoiceDocumentItem（請求伝票 - 明細データ）
* InvoiceDocumentItemPricingElement（請求伝票 - 明細価格決定要素データ）

## Output
data-platform-api-invoice-document-items-creates-subfunc-rmq-kube では、[golang-logging-library-for-data-platform](https://github.com/latonaio/golang-logging-library-for-data-platform) により、Output として、RabbitMQ へのメッセージを JSON 形式で出力します。以下の項目のうち、"cursor" ～ "time"は、golang-logging-library-for-data-platform による 定型フォーマットの出力結果です。
